...
struct is_unique<T0, T1, Rest...>
    : std::integral_constant<
              bool,
              !std::is_same<T0, T1>::value && is_unique<T0, Rest...>::value
...
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE2(                           \
    ...
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {           \
        using TestInit                                                        \
                = Catch::combine<INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>::   \
                        with_types<INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES( \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_2(                            \
    ...
    INTERNAL_CATCH_TEMPLATE_REGISTRY_INITIATE(                                 \
            TestNameClass, Name, __VA_ARGS__)                                  \
    }                                                                          \
    CATCH_INTERNAL_UNSUPPRESS_GLOBALS_WARNINGS                                 \
    template <typename TestType>                                               \
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...) \
    do {                                                       \
        ...
        INTERNAL_CATCH_REACT(catchAssertionHandler)            \
             ...
             (false)                                           \
                     && static_cast<bool>(                     \
                                !!(__VA_ARGS__))) // the expression here is
                                                  // never evaluated at runtime
                                                  // but it forces the compiler
                                                  // to give it a look
...
template <typename T>
typename std::enable_if<std::is_floating_point<T>::value, GeneratorWrapper<T>>::
        type
        random(T a, T b)
{
    return GeneratorWrapper<T>(
...
namespace Matchers {
namespace Impl { namespace NSStringMatchers {
...
private:
    void
    runCurrentTest(std::string& redirectedCout, std::string& redirectedCerr);
...
std::size_t listTests(Config const& config)
{
    ...
    for (auto const& testCaseInfo : matchedTestCases) {
        Colour::Code colour = testCaseInfo.isHidden() ? Colour::SecondaryText
                                                      : Colour::None;
...
namespace Catch { namespace Matchers { namespace Floating {
